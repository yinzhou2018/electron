...
const setupGuest = function (embedder, frameName, guest, options) {
  ...
  // yinzhou
  // 注释掉下面代码支持BrowserView对象充当guest角色，electron原生只支持BrowserWindow作为guest, 这无法满足多标签浏览器诉求
...
function internalWindowOpen(event, url, referrer, frameName, disposition, options, additionalFeatures, postData) {
  ...
  if (
    (event.sender.getType() === 'webview' && event.sender.getLastWebPreferences().disablePopups) ||
